...
# 定义主目录路径
log_root_dir = '/home/jingchen/clap-s/log-adpter/'
...
for shot_dir in os.listdir(log_root_dir):
    ...
    if os.path.isdir(full_shot_path) and shot_dir.endswith('shot'):
                ...
                with open(filepath, 'r') as file:
                    for line in file:
                        if 'Test Accuracy' in line:
                            ...
                            fiber_accuracies[f'seed{seed}'] = accuracy
                            break
            ...
            if coil_match:
                ...
                with open(filepath, 'r') as file:
                    for line in file:
                        if 'Test Accuracy' in line:
                            accuracy = float(line.split(': ')[1])
                            coil_accuracies[f'seed{seed}'] = accuracy
                            break
            ...
            if fiber_coil_match:
                ...
                with open(filepath, 'r') as file:
                    for line in file:
                        if 'Test Accuracy' in line:
                            accuracy = float(line.split(': ')[1])
                            fiber_coil_accuracies[f'seed{seed}'] = accuracy
                            break
            ...
            if fiber_mandrel_match:
                ...
                with open(filepath, 'r') as file:
                    for line in file:
                        if 'Test Accuracy' in line:
                            accuracy = float(line.split(': ')[1])
                            fiber_mandrel_accuracies[f'seed{seed}'] = accuracy
                            break
            ...
            if microphone_match:
                ...
                with open(filepath, 'r') as file:
                    for line in file:
                        if 'Test Accuracy' in line:
                            accuracy = float(line.split(': ')[1])
                            microphone_accuracies[f'seed{seed}'] = accuracy
                            break

        # 计算每个数据集的均值和方差并保存结果
        if fiber_accuracies:
            fiber_mean_accuracy = np.mean(list(fiber_accuracies.values()))
            fiber_std_accuracy = np.std(list(fiber_accuracies.values()))
        ...
        if coil_accuracies:
            coil_mean_accuracy = np.mean(list(coil_accuracies.values()))
            coil_std_accuracy = np.std(list(coil_accuracies.values()))
        ...
        if fiber_coil_accuracies:
            fiber_coil_mean_accuracy = np.mean(list(fiber_coil_accuracies.values()))
            fiber_coil_std_accuracy = np.std(list(fiber_coil_accuracies.values()))
        ...
        if fiber_mandrel_accuracies:
            fiber_mandrel_mean_accuracy = np.mean(list(fiber_mandrel_accuracies.values()))
            fiber_mandrel_std_accuracy = np.std(list(fiber_mandrel_accuracies.values()))
        ...
        if microphone_accuracies:
            microphone_mean_accuracy = np.mean(list(microphone_accuracies.values()))
            microphone_std_accuracy = np.std(list(microphone_accuracies.values()))
